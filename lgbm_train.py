...
def train(kpi_df, tuning=False, n_trials=20, norm=True, shuffle=False):
    params = {
    ...
    for kpi_id, df in groups:
        ...
        train_x, test_x, train_y, test_y = train_test_split(
            df, labels, shuffle=shuffle, test_size=0.2, random_state=666
        )
        ...
        if score < 0.2:
            ...
            print("Other", kpi_id, "f1:%.2f" % score)
        # print('LGBMPredictor', kpi_id, 'f1:%.2f' % score, cfg)
        else:
            print("LightGBM", kpi_id, "f1:%.2f" % score)
        scores.append(score)
...
if __name__ == "__main__":
    ...
    train(df, tuning=False, n_trials=20, norm=True, shuffle=False)
