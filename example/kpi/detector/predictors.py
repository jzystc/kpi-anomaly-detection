...
class LGBMPredictor:
    ...
    def train_model(self, series: pd.Series):
        """
        train the LGBM regressor model
        :return:
        """
        ## 历史模型训练
        samples = np.array([series.values[i - self.period - 1: i] for i in range(self.period + 1, len(series))])
        Xs = samples[:, :-1]
        Ys = samples[:, -1:]
        self.features = series[-self.period:]
        self.model = LGBM.LGBMClassifier(n_estimators=100, max_depth=5, random_state=0)
    ...
    def detect(self, series_new: pd.Series):
        ...
        for i in range(num_new):
            ...
            lower_bound = np.array(errs).mean() - self.sens * np.array(errs).std()
...
class XGBPredictor:
    ...
    def train_model(self, series: pd.Series):
        ...
        Ys = samples[:, -1:]
        self.features = series[-self.period:]
        self.model = XGB.XGBRegressor(n_estimators=100, max_depth=5, random_state=0)
        self.model.fit(Xs, Ys)
        self.errs = self.predict(list(series))
